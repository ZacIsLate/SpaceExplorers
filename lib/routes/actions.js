...
function resolveTalk(char, enemy, ship, spaceEnv, result){
    if(Math.floor(Math.random() * 100) - char.currentEvent.event.difficulty > 50){
        result.resolved = true;
        ...
        return Character.findByIdAndUpdate(char._id,
            ...
            if( ship.healthPoints < 1) { 
                result.resolved = true;
                result.lose = true;
                return `as you were trying to explain your peaceful intentions ${enemy.name} shoot you for ${enemy.damage + spaceEnv.damage} damage. died an idealist, till your last breath firmly believing that word is mightier then blaster. How wrong you were...`;
            }
    ...
    return Character.findByIdAndUpdate(char._id,   {'currentEvent.enemy.healthPoints':enemy.healthPoints}
        ...
        return Character.findByIdAndUpdate(char._id,
            ...
            if( ship.healthPoints < 1) { 
                result.resolved = true;
                result.lose = true;
                return `Your ship took ${enemyDmg} damage and was destroyed by ${enemy.name}. Try again! Or dont, you already bought the game.`;
            }
